...
class DetailSegue: UIStoryboardSegue {
    ...
    override func perform() {
        if source is EventsViewController {
            ...
            let sourceController = source as! ProfileViewController
            let indexPath = IndexPath(row: index, section: 0)
            let cell = sourceController.tableView.cellForRow(at: indexPath) as! EventCell
            ...
            cell.removeFromSuperview()
            cell.frame = cell.frame.offsetBy(dx: 0, dy: sourceController.tableView.frame.origin.y + sourceController.tableView.contentOffset.y)
            cell.backgroundColor = .clear
